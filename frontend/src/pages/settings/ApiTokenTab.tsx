...
export default function ApiTokenTab() {
  ...
  const [tokenSubjects, setTokenSubjects] = useState<ApiTokenSubject[]>([]);
  ...
  useEffect(() => {
    ...
    void HttpUtil.get<unknown>(TOKEN_SUBJECTS_ENDPOINT, undefined, { silent: true }).then((msg) => {
      const subjects = parseApiTokenSubjects(msg?.obj);
      if (msg?.success && subjects) setTokenSubjects(subjects);
    });
  }, [createOpen]);
    ...
    <div className="api-token-section">
      ...
      >
        ...
        >
          ...
          <Form.Item name="kind" label="RBAC-scoped user credential" initialValue="delegated">
            <Radio.Group options={[
              { value: 'delegated', label: 'User token' },
              { value: 'service', label: 'Service token' },
            ]} />
          </Form.Item>

          <Form.Item noStyle shouldUpdate={(previous, current) => previous.kind !== current.kind}>
            {({ getFieldValue }) => getFieldValue('kind') === 'delegated' ? (
              <>
                <Form.Item name="subjectAdminId" label="Panel administrator" rules={[{ required: true }]}>
                  <Select options={tokenSubjects.map((subject) => ({
                    value: subject.id,
                    label: `${subject.username} (${subject.roleName})`,
                  }))} />
                </Form.Item>
                <Form.Item name="scopes" label="Scopes" initialValue={['clients:read']}>
                  <Checkbox.Group options={[
                    { value: 'clients:read', label: 'Read clients' },
                    { value: 'clients:create', label: 'Create clients' },
                    { value: 'custom-panel:manage', label: 'Custom panel bot' },
                  ]} />
                </Form.Item>
              </>
            ) : (
