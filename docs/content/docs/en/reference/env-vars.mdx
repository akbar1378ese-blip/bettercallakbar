...
description: Complete reference for bettercallakbar's XUI_* environment variables — database, panel, logging, memory, and the tunnel health monitor.
icon: Variable
---

bettercallakbar reads its runtime configuration from `XUI_*` environment variables. On a
