...
description: Shape egress in bettercallakbar — WARP and NordVPN outbounds, outbound subscriptions (server pools), routing rules, and load balancers.
icon: Route
---

Inbounds accept clients; **outbounds** decide where their traffic goes next.
bettercallakbar can route traffic through Cloudflare WARP, NordVPN, or arbitrary outbound
...
shapes bettercallakbar uses: **VLESS is the flat form** (`address`/`port`/`id`/`flow`/
...
WARP lets your server egress through Cloudflare's network. bettercallakbar can register a
...
From the panel's WARP controls, register an account. bettercallakbar fills the outbound's
...
bettercallakbar can fetch NordVPN (NordLynx/WireGuard) credentials from an access token (or
