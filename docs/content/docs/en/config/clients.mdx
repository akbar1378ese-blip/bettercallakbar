---
title: Clients
description: Manage bettercallakbar clients — credentials, traffic and expiry limits, IP limits, groups, bulk actions, external links, and online status.
icon: Users
---
...
| **Limit IP**   | all                   | Max simultaneous source IPs (enforced by the bettercallakbar custom core).        |
...
- **Limit IP** caps simultaneous source IPs inside the bettercallakbar custom core, before outbound dispatch. You can view a client's recent IPs and clear them from the client's actions.
